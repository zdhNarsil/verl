...
class RayPPOTrainer:
    ...
    def init_workers(self):
        ...
        if self.hybrid_engine:
            ...
            self.rollout_wg = all_wg["actor_rollout"]
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            batch, batch_next, gen_batch_output, gen_batch_output_next = None, None, None, None
            gen_batch, gen_batch_next = None, None
            iterator = iter(self.train_dataloader)

            def gen_next_batch(iterator):
                nonlocal batch, batch_next, gen_batch_output, gen_batch_output_next, gen_batch, gen_batch_next
                # waiting for the output of previous rollout step
                if batch_next is not None:
                    assert gen_batch_output_next is not None
                    assert gen_batch_next is not None
                    gen_batch_output = gen_batch_output_next.get()
                    batch, gen_batch = batch_next, gen_batch_next
                    batch_next, gen_batch_output_next = None, None
                ...
                batch_next = DataProto.from_single_dict(batch_dict)
                # pop those keys for generation
                batch_keys_to_pop = ["input_ids", "attention_mask", "position_ids"]
                non_tensor_batch_keys_to_pop = ["raw_prompt_ids"]
                if "multi_modal_data" in batch_next.non_tensor_batch:
                    non_tensor_batch_keys_to_pop.append("multi_modal_data")
                if "raw_prompt" in batch_next.non_tensor_batch:
                    non_tensor_batch_keys_to_pop.append("raw_prompt")
                if "tools_kwargs" in batch_next.non_tensor_batch:
                    non_tensor_batch_keys_to_pop.append("tools_kwargs")
                if "interaction_kwargs" in batch_next.non_tensor_batch:
                    non_tensor_batch_keys_to_pop.append("interaction_kwargs")
                gen_batch_next = batch_next.pop(
                    batch_keys=batch_keys_to_pop,
                    non_tensor_batch_keys=non_tensor_batch_keys_to_pop,
                )

                # async generation
                gen_batch_output_next = self.rollout_wg.generate_sequences(gen_batch_next)

            gen_next_batch(iterator)
            while batch_next is not None:
